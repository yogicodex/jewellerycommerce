...
<v-carousel :images="{{ json_encode($options['images'] ?? []) }}">
    <div class="overflow-hidden">
        {{-- Adjusted the aspect ratio and added max-h-[800px] --}}
        <div class="shimmer aspect-[2.4/1] w-screen max-h-[800px]"></div>
    </div>
</v-carousel>
...
@pushOnce('scripts')
    ...
    >
        {{-- Added max-h-[800px] to the main container --}}
        <div class="relative m-auto flex w-full overflow-hidden max-h-[800px]">
            <!-- Slider -->
            <div
            ...
            >
                <div
                    {{-- The w-screen and added object-cover with a specific aspect ratio will handle the sizing --}}
                    class="w-screen bg-no-repeat bg-cover"
                    v-for="(image, index) in images"
                    @click="visitLink(image)"
                    ref="slide"
                >
                    <x-shop::media.images.lazy
                        {{-- Adjusted classes for aspect ratio and max height --}}
                        class="aspect-[2.4/1] max-h-full w-full max-w-full select-none transition-transform duration-300 ease-in-out object-cover"
                        ::lazy="false"
                        ::src="image.image"
