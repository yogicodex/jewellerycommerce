<x-admin::layouts.anonymous>
    ...
    <div class="flex h-[100vh] items-center justify-center bg-white dark:bg-gray-900">
        <div class="flex max-w-[745px] items-center gap-5">
            <div class="w-full">
                @php
                    $logoUrl = core()->getConfigData('general.design.admin_logo.logo_image')
                        ...
                        : bagisto_asset('src/Resources/assets/images/logo.svg');
                @endphp

                <img class="h-10 mb-6" src="{{ $logoUrl }}" id="logo-image" alt="{{ config('app.name') }}" />
            ...
            <div class="w-full">
                <img src="{{ bagisto_asset('src/Resources/assets/images/error.svg') }}" />
            </div>
        </div>
    </div>
</x-admin::layouts.anonymous>
